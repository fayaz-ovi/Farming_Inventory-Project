...
if (isset($_GET['table'])) {
    ...
    <form method='GET' class='search-bar'>
        ...
        <input type='text' name='search' placeholder='Search by customer_id or farmer_id'>
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        ...
        if ($table === 'farmer') {
            $search_query = "WHERE users.customer_id LIKE '%$search%' OR users.name LIKE '%$search%'";
        } elseif ($table === 'product') {
            $search_query = "WHERE PRODUCT_ID LIKE '%$search%'";
        } elseif ($table === 'checkout') {
            $search_query = "WHERE checkout_id LIKE '%$search%' OR user_id LIKE '%$search%'";
        }
    }

    if ($table === 'farmer') {
        // Join with users to fetch user_id as farmer_id
        $result = $mysqli->query("SELECT farmer.*, users.customer_id AS farmer_id, users.name FROM farmer JOIN users ON farmer.user_id = users.customer_id $search_query");
    } elseif ($table === 'product') {
        $result = $mysqli->query("SELECT * FROM product $search_query");
    } elseif ($table === 'checkout') {
        $result = $mysqli->query("SELECT checkout.*, users.customer_id AS customer_id FROM checkout JOIN users ON checkout.user_id = users.customer_id $search_query");
    }
    ...
    if ($result && $result->num_rows > 0) {
        ...
        foreach ($fields as $field) {
            // Exclude PASSWORD and original user_id from display
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($table === 'product') {
                ...
                echo "<td><a href='?delete_farmer={$row['farmer_id']}'>Delete</a></td>"; // Use farmer_id for deletion
            } elseif ($table === 'checkout') {
                echo "<td><a href='?delete_checkout={$row['checkout_id']}'>Delete</a></td>";
            }
            echo "</tr>";
        }
        echo "</table>";
...
if (isset($_GET['delete_farmer'])) {
    ...
    $mysqli->query("DELETE  FROM product WHERE user_id='$farmer_id'");
    $mysqli->query("DELETE FROM users WHERE customer_id='$farmer_id'");
    header('Location: ' . $_SERVER['PHP_SELF']);
    exit();
}
...
if (isset($_GET['delete_checkout'])) {
    $checkout_id = $_GET['delete_checkout'];
    $mysqli->query("DELETE FROM checkout WHERE checkout_id='$checkout_id'");
    header('Location: ' . $_SERVER['PHP_SELF']);
    exit();
}
?>
